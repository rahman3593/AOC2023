...
{
    private static readonly string[] Input =  //79315
    {
    ...
    // private static readonly string[] Input = //2286
    // {
    ...
    public static int Calculation()
    {
        ...
        {
            var dictionary = keyValuePair.Value.SelectMany(x => x).GroupBy(x => x.Key)
                .ToDictionary(x => x.Key, x => x.Max(y => y.Value));
            result.Add(Convert.ToInt32(keyValuePair.Key), dictionary);
        }

        return result.Sum(keyValuePair => keyValuePair.Value.Select(x => x.Value).Aggregate((x, y) => x * y));
    }
}