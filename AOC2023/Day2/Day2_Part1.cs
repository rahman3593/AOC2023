...
public static class Day2Part1
{
    ...
    private static readonly string[] Input = //ANSWER: 2085
    {
    ...
    // private static readonly string[] Input = /ANSWER: 8
    // {
    ...
    {
        Dictionary<string, IEnumerable<Dictionary<string, int>>> parsedInput = new();

        var inputDict = Input.Select(inputString => inputString.Split(":"))
            .ToDictionary(strings => strings[0].Replace("Game ", string.Empty), strings => strings[1]);

        foreach (var keyValuePair in inputDict)
        {
            var setOfCubes = keyValuePair.Value.Split(";")
                .Select(sets => sets.Trim()).ToList()
                .Select(set => set.Split(",", StringSplitOptions.TrimEntries).ToList())
                .Select(cubes => cubes.Select(colourCount => colourCount.Split(" "))
                    .ToDictionary(cubeColour => cubeColour[1],
                        cubeCount => Convert.ToInt32(cubeCount[0])));

            parsedInput.Add(keyValuePair.Key, setOfCubes);
        }

        var invalidRecords = parsedInput.Where(x => x.Value.Any(y =>
                y.Any(z => z.Key.Equals("red") && z.Value > RedCount)
                || y.Any(z => z.Key.Equals("blue") && z.Value > BlueCount)
                || y.Any(z => z.Key.Equals("green") && z.Value > GreenCount)))
            .Select(x => x.Key).ToList();


        return parsedInput.Where(x => !invalidRecords.Contains(x.Key)).ToList().Select(x => x.Key).ToList()
            .Sum(Convert.ToInt32);
    }
}